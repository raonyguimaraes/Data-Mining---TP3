...
class RCluster:
  ...
  def main(self):
    ...
    distances = self.distance_matrix(musics)
    #distances = []
    #print "Relevant Musics..."
    #self.mostrelevant(samples)
    print "Cluster Items..."
    self.cluster(musics, distances)
  ...
  def distance_matrix(self, items):
	...
	if idx1 <= idx2:
	  ...
	  distances[idx1][idx2] = score
	if idx2 > idx1:
	  distances[idx2][idx1] = distances[idx1][idx2]
	
    #print distances
    #for idx1, dist1 in enumerate(distances):
      #for idx2, dist2 in enumerate(dist1):
	#print distances[idx1][idx2],
      #print
  ...
  def cluster(self, items, distances):
    ...
    Y = sch.linkage(D, method='single', metric='cosine')#, metric=''
    Y[abs(Y)<3e-16] = 0.0
    treshold = 0.0
    Z = sch.dendrogram(Y, orientation='top', color_threshold=treshold) #, color_threshold=treshold
        
    
    #treshold_list = [0.01, 0.1, 0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 0.95, 0.99, 0.999, 0.9999, 0.99999]
    #for treshold in treshold_list:
      #print "Usando treshold: "+str(treshold)
      #T = sch.fcluster(Y, treshold, criterion='distance')  # n
	...
	if cluster_size > 1:
	  ...
	  cluster_distance = cluster_distance / cluster_size
